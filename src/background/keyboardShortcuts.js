...
export const initShortcuts = async () => {
  ...
  for (const commandId of Object.keys(commands)) {
    ...
    } catch (e) { log.error(logDir, "initShortcuts()", e); }
  }
  setSettings("initedShortcuts", initedShortcuts);
};
export const onCommandListener = async command => { log.log(logDir, "onCommandListener()", command);
  switch (command) {
    case "translateSelectedText": { translateSelectedText(); break; }
    case "translatePage": { translatePage(); break; }
  }
};
const translateSelectedText = async () => {
  const tab = (await browser.tabs.query({ active: true }))[0];
  browser.tabs.sendMessage(tab.id, { message: "translateSelectedText" });
};
